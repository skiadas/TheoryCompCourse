...
> The language $B=\left\{ a^nb^nc^n\mid n\geq 0 \right\}$ is not context-free. In general, any languages that requires three parts to all change together is likely to not be context-free.
...
This means that $uv^2xy^2z\in B$ has either the same number of $a$s or the same number of $c$s, while one of the other symbols' count goes up. This is a contradiction.
...
- We have a finite set of states $Q$. And we transition between them.
...
> - $\Gamma$ is the (finite) set of possible stack symbols (symbols that can be stored in the stack). Typically this will include the terminals, the nonterminals and occasionally other special symbols.
...
> - Add a new start state, with a transition $\epsilon,\epsilon\to\$$ to the previous start state. This adds this special symbol at the bottom of the stack. Running into that symbol means the stack has been emptied.
...
In fact this chain of four states a very common pattern for pushdown automata. What changes from example to example is the transitions around $q_1$ and $q_2$.
...
bbaabba     Sa$       a,a   -> eps
bbaabba     bSba$     eps,S -> bSb                                => abSba
baabba      Sba$      b,b   -> eps
baabba      bSbba$    eps,S -> bSb                                => abbSbba
aabba       Sbba$     b,b   -> eps
aabba       aSabba$   eps,S -> aSa                                => abbaSabba
abba        Sabba$    a,a   -> eps
